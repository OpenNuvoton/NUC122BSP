...
void USBD_GetDescriptor(void)
{
    ...
    switch(g_usbd_SetupPacket[3])
    {
        ...
        case 15: // Get BOS descriptor
        {
            uint32_t u32TotalLen;

            u32TotalLen = g_usbd_sInfo->gu8ConfigDesc[12];
            u32TotalLen = g_usbd_sInfo->gu8ConfigDesc[11] + (u32TotalLen << 8);

            DBG_PRINTF("Get BOS desc len %d, acture len %d\n", u32Len, u32TotalLen);
            u32Len = Minimum(u32Len, u32TotalLen);
            DBG_PRINTF("Minimum len %d\n", u32Len);
            USBD_PrepareCtrlIn((uint8_t *)&g_usbd_sInfo->gu8ConfigDesc[9], u32Len);
            USBD_PrepareCtrlOut(0, 0);
            break;
        }
